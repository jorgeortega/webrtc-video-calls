...
export function useWebRTC({
  ...
  const setupDataChannel = useCallback(
    (channel: RTCDataChannel, remotePeerId: string) => {
      channel.onopen = () => {
  ...
  const createPeerConnection = useCallback(
    async (
      ...
      pc.onconnectionstatechange = () => {
        console.log(
          `Connection state with ${remotePeerId}: ${pc.connectionState}`,
        );
  ...
  const handleOffer = useCallback(
    async (
      ...
      if (isNewConnection) {
        ...
        setRemotePeers((prev) =>
          prev.map((p) =>
            p.peerId === remotePeerId && p.username === "Unknown"
  ...
  const updatePeerUsername = useCallback(
    (peerId: string, newUsername: string) => {
      setRemotePeers((prev) =>
        prev.map((p) =>
          p.peerId === peerId ? { ...p, username: newUsername } : p,
        ),
      );
    },
    [],
